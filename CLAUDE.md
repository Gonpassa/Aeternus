...
**Phases 1-5 are complete.** The npm-workspaces monorepo skeleton, backend API core (Postgres/Drizzle, sessions), authentication (Passport session auth, protected routes, client `AuthProvider`), the journal module (entries CRUD, mood tagging, calendar filter, rich text editor), and production deployment (Fly.io app `aeternus`, single Docker image, self-managed Postgres, Mongo-to-Postgres data migration) are all built, tested, and live. Journal insights/overview is deferred to a later pass. `backend/src/modules/` and `client/src/modules/` currently hold only the `journal` module — Dream Journal, Structured Writing, Calendar, and AI-assisted learning (Phases 6-9) haven't started. Don't design those later-phase data models before their phase starts.

See `docs/adr/0001-npm-workspaces-monorepo.md` for the Phase 1 architecture rationale, `docs/deploy-smoke-test.md` and `docs/data-migration-runbook.md` for the Phase 5 deploy process, and `README.md` for the full module descriptions and 9-phase build plan (Setup → Backend API core → Auth → Journal → Deployment → Dream Journal → Structured Writing → Calendar → AI learning). Follow that phase order rather than jumping ahead.
