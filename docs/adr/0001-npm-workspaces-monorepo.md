...
Aeternus is a deliberate departure from Nee.2, which kept `client/` and `backend/` as fully independent packages with no root tooling.
Aeternus instead uses an npm workspaces monorepo (`"workspaces": ["client", "backend", "packages/*"]`) so client, backend, and `packages/shared-types` share one root `tsconfig.base.json`, one install step, and one root lint/build invocation, instead of keeping two copies of tooling config in sync by hand.
